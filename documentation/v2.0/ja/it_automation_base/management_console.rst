...
- | **プルダウン選択を含んだ場合の変更履歴について**
  ...
  #. | 事前準備として、メニュー作成メニューグループ>メニュー定義・作成メニューで以下のデータシートとパラメータシートを作成します。
     ...
     .. figure:: ./management_console/パラメータシート「ぱらむ 001」.png
  ...
  #. | 入力用メニューグループ>マスタ001メニューからパラメータ「マスタ」の値を編集し「mas1-2」で編集反映を行います。
  #. | 入力用メニューグループ>マスタ001メニューからパラメータ「マスタ」の値を編集し「mas1-3」で編集反映を行います。
  #. | 入力用メニューグループ>ぱらむ001メニューから先ほど登録した対象の「ぱらむA」を編集し、編集反映を行います。
  #. | 入力用メニューグループ>マスタ001メニューからパラメータ「マスタ」の値を編集し「mas1-4」で編集反映を行います。
  #. | 入力用メニューグループ>マスタ001メニューからパラメータ「マスタ」の値を編集し「mas1-5」で編集反映を行います。
  #. | 入力用メニューグループ>ぱらむ001メニューから先ほど登録した対象の「ぱらむA」を編集し、編集反映を行います。
...
| **{{#3:下記の「テーブル設定」「ヘッダー固定設定」「サブメニュー初期状態設定」について、2系では実装されていない認識で合っているでしょうか？}}**
テーブル設定
************
...
| **{{#9:「メニューグループに対するメニューの確認」「メニュー管理への遷移」は実装されていない認識で合っているでしょうか。}}**
...
| **{{#4:下記「warning」の内容について、「メニューグループの登録を行うと...」という内容は正しいでしょうか。}}**
...
- | **【パネル用画像】**
  ...
  | **{{#5:パネル画像エディタ機能は実装されていない認識で合っているでしょうか。}}**
...
| **{{#6:「メニュグループ管理への遷移」、「メニューに対するロール情報の確認」、「ロール管理への遷移機能」は実装されていない認識で合っているでしょうか。}}**

#. | メニューグループ管理への遷移
   | メニューグループIDまたはメニューグループ名称のリンクをクリックして、対象のメニューグループ管理へ遷移します。
#. | メニューに対するロール情報の確認
...
#. | ロール管理への遷移
   | ロールIDまたはロール名称のリンクをクリックして、対象のロール管理へ遷移します。
...
#. | パラメータの設定
   ...
   | **{{#6:[7 ソートキー]の入力内容について、項目名は「ASC/DESC」であっているでしょうか。また、カラム名に関してはどこから参照したらよいでしょうか。}}**
   ...
   | **{{#7:2系はダウンロード項目の変化ではなく、ダウンロードの中止であっているでしょうか。}}**
...
| **{{#8:「ロール管理への遷移」「メニューグループ管理への遷移」「メニュー管理への遷移」は実装されていない認識で合っているでしょうか。}}**
#. | ロール管理への遷移
...
ファイル項目-ファイル削除機能
*****************************
| 必須でないファイル項目において、ファイルクリアした状態で編集反映を行うと、登録済みのファイルが削除されます。（必須項目の場合はチェックボックスが非表示となります。）
...
| **{{#10:2系の実装内容と下記章「BackYardコンテンツ」の内容があっているかご確認お願いします。}}**
